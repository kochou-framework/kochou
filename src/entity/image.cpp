...
kochou::entity::image::make(kochou::shared_context _sctx, kochou::entity::swapchain & _swapchain) noexcept
{
    ...
    ktl::u32 amount = 0;
    auto     rc     = ktl::api::get_swapchain_images_khr(device, _swapchain.swapchain_, &amount, nullptr);
    ...
    rc = ktl::api::get_swapchain_images_khr(device, _swapchain.swapchain_, &amount, raw_images.data());
    if (rc != ktl::api::result::v_success)
    {
        return ktl::err(ktl::cast_api_result(rc));
    }
